...
- **Travel consultant** leaderboard/profile: `travel-consultant-server-loader.ts` + fixed current-year queries; pages consume `initialSnapshot` directly (no leaderboard/profile refresh hooks). Leaderboard table uses **`travelRankings`** with **Travel Revenue** and **Travel Gross Profit** (`sortBy` default `travel_revenue`) — **travel-start-date** semantics via `vw_travel_consultant_travel_monthly_v2`, including future **active** months in the underlying rollups. Profile trend matrices: **Travel Gross Profit**, **Travel Revenue** (travel-start), and **Booked Revenue** (close-date; matrix years stop at the current calendar year). Profile header `ytdVariancePct` remains booking-pace booked revenue (see backend docs). Profile **Performance** card no longer duplicates the forecast timeline table (timeline remains available via embedded profile payload / backend; standalone `GET …/forecast` remains unused by UI—see `frontend-data-queries.md`).
...
- Salesforce ingestion operator visibility is frontend-mediated through the same control-plane surfaces (no direct frontend-to-Salesforce calls):
  - monitor `salesforce-data-ingestion-sync` status from `data-jobs/health` and run-feed filters
  - trigger manual run replay only through `POST /api/v1/data-jobs/salesforce-data-ingestion-sync/runs`
